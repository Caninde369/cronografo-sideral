...
const HouseCuspCard: React.FC<{
    ...
    isCompact: boolean;
    showCuspDegrees?: boolean;
}> = ({ houseNumber, longitude, isSelected, isSecondarySelected, onHover, onClick, onRightClick, houseFormat, scale, zodiacColorMode, layoutMode, isCompact, showCuspDegrees }) => {
    ...
    const itemSize = (isCompact ? 14 : 16) * scale;
        ...
        return (
            ...
            >
                <div className="flex justify-between items-start">
                    <span className={`font-bold text-brand-text ${houseFormat === 'roman' ? 'font-display' : 'font-ui'}`} style={{ fontSize: `${fontSize * 1.2}px` }}>
                        {label}
                    </span>
    ...
    return (
        <div 
            className={`
                flex items-center justify-between ${isCompact ? 'px-1 py-1' : 'px-2 py-1.5'} rounded-xl border transition-all duration-200 cursor-pointer group w-full font-manrope
                ${isSelected 
        ...
        >
            <div 
                className={`font-bold min-w-[1.5rem] text-center ${isSelected ? 'text-violet-300' : 'text-brand-text-muted'} ${houseFormat === 'roman' ? 'font-display' : 'font-ui'}`}
            ...
            <div className="flex items-center justify-center flex-1 min-w-0 gap-1">
                ...
                {showCuspDegrees && (
                    <span className="font-manrope font-medium text-brand-text/70 tabular-nums" style={{ fontSize: `${degreeSize * 0.9}px` }}>
                        {formatDegrees(longitude % 30)}
                    </span>
                )}
            </div>
...
export const HouseCuspsPanel: React.FC<HouseCuspsPanelProps> = ({
    ...
    useEffect(() => {
        if (onWidthChange) {
            const baseWidth = layoutMode === 'complete' ? 230 : 160;
    ...
    return (
        <div className="bg-brand-surface/40 backdrop-blur-xl rounded-3xl shadow-sm flex flex-col h-auto border border-brand-border/8 ring-1 ring-white/5 font-manrope w-full">
            <div className="px-3 py-2 border-b border-brand-border/5 flex justify-between items-center gap-2 flex-shrink-0 bg-brand-surface-highlight/30 rounded-t-3xl">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                    {dragHandle}
                    <h3 className="text-[10px] font-medium uppercase text-brand-text-muted tracking-wider truncate font-outfit">Casas</h3>
            ...
            {isMenuOpen && (
                <PopupMenu onClose={() => setActiveMenu(null)} triggerRef={menuButtonRef}>
                    <div className="flex flex-col gap-3">
                        <button
                            onClick={(e) => { e.stopPropagation(); onToggleCuspDegrees?.(); setActiveMenu(null); }}
                            className="w-full px-3 py-2 rounded-lg flex items-center justify-between transition-colors hover:bg-brand-surface-highlight text-brand-text-muted hover:text-brand-text"
                        >
                            <span className="text-xs font-manrope text-brand-text-muted/60">Mostrar graus</span>
                            <div className={`w-6 h-3 rounded-full relative transition-colors ${showCuspDegrees ? 'bg-brand-purple' : 'bg-brand-surface-highlight'}`}>
                                <div className="absolute top-0.5 w-2 h-2 rounded-full bg-white transition-all" style={{left: showCuspDegrees ? 'calc(100% - 10px)' : '2px'}} />
                            </div>
                        </button>
                ...
                <div className={`flex-1 min-w-0 flex flex-col ${layoutMode === 'complete' ? 'gap-2' : (isCompact ? 'gap-0.5' : 'gap-1')}`}>
                    {leftColumnHouses.map((longitude, index) => (
                        <HouseCuspCard
                            ...
                            isCompact={isCompact}
                            showCuspDegrees={showCuspDegrees}
                        />
                    ))}
                </div>
