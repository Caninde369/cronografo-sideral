...
import { SeasonIngress } from '../lib/astrology/calendars';
...
export const SiderealClock: React.FC<SiderealClockProps> = ({
    ...
    // getNatalAngle: âncora no ASC natal para posicionamento correto no bi-wheel
    const getNatalAngle = React.useCallback((longitude: number) => {
        if (!natalData) return normalizeAngle(180 - longitude);
        const anchor = isZodiacFixed ? 0 : natalData.ascendantLongitude;
        return normalizeAngle(180 - (longitude - anchor));
    }, [natalData, isZodiacFixed]);
...
const natalPlanetPositions = useMemo(() => {
    ...
    Object.keys(visiblePlanets).forEach(planetId => {
        ...
        if (longitude !== undefined) {
            // Usar getNatalAngle — âncora no ASC natal para posicionamento correto
            const angle = getNatalAngle(longitude);
            const r = getPlanetRadius(planetId);
            const pos = polarToCartesian(cx, cy, r, angle);
            positions[planetId] = { x: pos.x, y: pos.y, angle };
        }
    ...
    return positions;
}, [natalData, visiblePlanets, getNatalAngle]);
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        mousePos.current = { x: e.clientX, y: e.clientY };
        if (tooltip && !tooltipRafRef.current) {
            tooltipRafRef.current = requestAnimationFrame(() => {
                setTooltip(prev => prev
                    ? { ...prev, x: mousePos.current.x, y: mousePos.current.y }
                    : null
                );
...
fontStyle: 'italic',
        ...
        return elements;
        // minuteKey: atualiza o anel 1x/minuto em vez de 1x/segundo
        // O anel 24h mostra labels de hora — precisão de minuto é suficiente
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [midheavenLongitude, ascendantLongitude, descendantLongitude, imumCoeliLongitude,
        isZodiacFixed, showTimeRing, dynamicContrastFill, isLightBackground,
        celestialData.sunEclipticLongitude,
        Math.floor((currentTime?.getTime() ?? 0) / 60000), // ← minuteKey
        timeRingScale, theme]);


    // Rotação do anel natal: diferença entre ASC natal e ASC trânsito
    // Isso faz o anel natal girar para mostrar o zodíaco do ponto de vista natal
    const natalZodiacRotation = useMemo(() => {
        if (!natalData || !isNatalMode) return 0;
        return normalizeAngle(natalData.ascendantLongitude - ascendantLongitude);
    }, [natalData, isNatalMode, natalData?.ascendantLongitude, ascendantLongitude]);

    // Ângulos das cúspides de casas natais para o overlay
    const natalHouseCuspAngles = useMemo(() => {
        if (!natalData || !isNatalMode) return [];
        return natalData.houseCusps.map((cusp: number) => getAngle(cusp));
    }, [natalData, isNatalMode, ascendantLongitude, isZodiacFixed]);
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" className="w-full h-full font-serif select-none overflow-visible">
                <defs>
                    ...
                    {showConstellations && (
                        <g className="pointer-events-none" transform={`rotate(${isZodiacFixed ? 0 : zodiacRotation}, ${cx}, ${cy})`}>
                            <image
                                ...
                                width="1875"
                                height="1875"
                                opacity={theme === 'light' ? 0.18 : 0.4}
                                style={{ 
                                    mixBlendMode: theme === 'light' ? 'multiply' : 'screen',
                                    filter: theme === 'light' 
                                        ? 'sepia(1) saturate(0.3) hue-rotate(190deg) brightness(0.4)' 
                            ...
                            <g className="pointer-events-none">
                                 ...
                                 <g transform={`translate(${sunriseLabelPos.x}, ${sunriseLabelPos.y})`}>
                                     ...
                                     <text 
                                        ...
                                        className="text-[14px] font-bold fill-brand-text font-display tracking-wider"
                                     >
                ...
                <g className="text-gray-300 pointer-events-none" transform={`rotate(${isZodiacFixed ? 0 : zodiacRotation}, ${cx}, ${cy})`}>
                    <circle 
                        ...
                        fill="none" stroke="currentColor" className="text-gray-800" style={{ opacity: 0.12 }} strokeWidth={graduationTickLength}
                    />
                    <circle cx={cx} cy={cy} r={graduationRingRadius} fill="none" stroke="currentColor" className="text-gray-300" strokeWidth="0.8"/>
                    <circle cx={cx} cy={cy} r={graduationRingInner} fill="none" stroke="currentColor" className="text-gray-300" strokeWidth="0.8"/>
                    {graduationTicks}
                </g>
                ...
                {/* ── ANEL ZODIACAL NATAL (bi-wheel) ── */}
                {isNatalMode && natalData && (
                    <g className="pointer-events-none">
                        <g transform={`rotate(${isZodiacFixed ? 0 : (zodiacRotation + natalZodiacRotation)}, ${cx}, ${cy})`}>

                            {/* Faixas alternadas dos 12 signos natais */}
                            {ZODIAC_DATA.map((sign, i) => {
                                const startAngle = 180 - (i * 30);
                                const endAngle   = 180 - ((i + 1) * 30);
                                // Anel entre r=418 e r=436 — abaixo do anel de graduação (438)
                                const wedge = describeAnnularSector(
                                    cx, cy, 418, 436,
                                    normalizeAngle(endAngle), normalizeAngle(startAngle)
                                );

                                // Abreviações PT-BR dos signos
                                const SIGN_ABBR: Record<string, string> = {
                                    aries:       'ARI', taurus:      'TAU', gemini:      'GEM',
                                    cancer:      'CAN', leo:         'LEO', virgo:       'VIR',
                                    libra:       'LIB', scorpio:     'ESC', sagittarius: 'SAG',
                                    capricorn:   'CAP', aquarius:    'AQU', pisces:      'PEI',
                                };

                                // Centro do setor para posicionar o texto
                                const midAngle = normalizeAngle(180 - (i * 30) - 15);
                                const textPos  = polarToCartesian(cx, cy, 427, midAngle);
                                ...
                                return (
                                    <g key={`natal-ring-${i}`}>
                                        ...
                                        {/* Abreviação do signo — âmbar suave, 8px */}
                                        <text
                                            x={textPos.x}
                                            y={textPos.y}
                                            textAnchor="middle"
                                        ...
                                        >
                                            {SIGN_ABBR[sign.id] ?? sign.id.slice(0,3).toUpperCase()}
                                        </text>
                                    </g>
                                );
                            })}

                            {/* Traços divisores dos 12 signos */}
                            {Array.from({ length: 12 }, (_, i) => {
                                const angle = 180 - (i * 30);
                                const p1 = polarToCartesian(cx, cy, 418, angle);
                                const p2 = polarToCartesian(cx, cy, 436, angle);
                                return (
                                    <line
                                        key={`natal-sign-tick-${i}`}
                                        x1={p1.x} y1={p1.y}
                                        x2={p2.x} y2={p2.y}
                                        stroke="#FFE566"
                                        strokeWidth={0.8}
                            ...
                            {/* Bordas do anel natal */}
                            <circle cx={cx} cy={cy} r={436}
                                fill="none" stroke="#FFE566"
                                strokeWidth={0.8} strokeOpacity={0.40}
                            />
                            <circle cx={cx} cy={cy} r={418}
                                fill="none" stroke="#FFE566"
                        ...
                        {/* Label "N" identificando o anel natal — fora da rotação */}
                        <text
                            x={cx}
                            y={cy - 442}
                            textAnchor="middle"
                ...
                {/* ── FIM ANEL ZODIACAL NATAL ── */}

                {showSeasonsRing && !isNatalMode && seasonIngresses && seasonIngresses.length === 4 && (
                    <g className="pointer-events-none select-none" transform={`rotate(${isZodiacFixed ? 0 : zodiacRotation}, ${cx}, ${cy})`}>
                        {seasonIngresses.map((season, i) => {
                            const nextSeason = seasonIngresses[(i + 1) % 4];
                            
                            // start e end são longitudes eclípticas (0-360)
                            const startLon = season.longitude;
                            const endLon = nextSeason.longitude;

                            // Converter longitude eclíptica para ângulo SVG
                            // No SVG: angle = 180 - longitude (zodíaco começa à esquerda)
                            const startAngle = 180 - startLon;
                            const endAngle = endLon === 0 ? 180 - 360 : 180 - endLon;

                            const midLon = startLon + (endLon > startLon
                                ? (endLon - startLon) / 2
                                : (360 - startLon + endLon) / 2);
                            const midAngle = normalizeAngle(180 - midLon);

                            const isLight = theme === 'light';
                            const lightColors = isLight
                                ? SEASON_COLORS_LIGHT[season.name as keyof typeof SEASON_COLORS_LIGHT]
                                : null;
                            ...
                            return (
                                <g key={`season-${i}`}>
                                    <path d={wedgePath} fill={fillColor} fillOpacity={fillOpacity}
                                        stroke={strokeColor} strokeOpacity={strokeOpacity} strokeWidth="1" />
                                    <path id={textPathId} d={textPathD} fill="none" stroke="none" />
                                    <text fill={textColor} fontSize="10.5" fontFamily="sans-serif"
                                        fontWeight="600" letterSpacing="2" className="opacity-80">
                ...
                {/* ── OVERLAY CASAS NATAIS ── */}
                {isNatalMode && natalData && natalHouseCuspAngles.length === 12 && (
                    <g className="pointer-events-none">
                        {natalHouseCuspAngles.map((angle, i) => {
                            const houseNumber = i + 1;
                            const isMainAngle = i === 0 || i === 3 || i === 6 || i === 9;

                            // Linha da cúspide natal — do centro até o anel zodiacal interno
                            const posInner = polarToCartesian(cx, cy, innerVoidRadius, angle);
                            const posOuter = polarToCartesian(cx, cy, zodiacRingInner, angle);

                            // Label da casa natal — posicionado levemente mais próximo ao centro
                            // que as casas de trânsito para não colidir
                            const natalLabelRadius = innerVoidRadius + 18;
                            const nextAngle = natalHouseCuspAngles[(i + 1) % 12];
                            let midAngleDeg = angle + (
                                (() => {
                            ...
                            midAngleDeg = normalizeAngle(midAngleDeg);
                            const labelPos = polarToCartesian(cx, cy, natalLabelRadius, midAngleDeg);

                            // Label da cúspide (ASC, MC, IC, DSC ou número)
                            let cuspLabel = '';
                            if (houseNumber === 1) cuspLabel = 'A';
                            else if (houseNumber === 10) cuspLabel = 'M';
                            else if (houseNumber === 4) cuspLabel = 'I';
                            else if (houseNumber === 7) cuspLabel = 'D';

                            return (
                                <g key={`natal-house-${i}`}>
                                    {/* Linha da cúspide natal */}
                                    <line
                                        x1={posInner.x} y1={posInner.y}
                                        x2={posOuter.x} y2={posOuter.y}
                                        stroke="#FFE566"
                                        strokeWidth={isMainAngle ? 1.2 : 0.6}
                                        strokeOpacity={isMainAngle ? 0.7 : 0.35}
                                        strokeDasharray={isMainAngle ? "6 4" : "3 4"}
                                    />

                                    {/* Marcador pequeno na borda interna das cúspides principais */}
                                    {isMainAngle && (
                                        <circle
                                            cx={posInner.x}
                                            cy={posInner.y}
                                    ...
                                    {/* Label das cúspides principais (A=ASC, M=MC, I=IC, D=DSC) */}
                                    {cuspLabel && (
                                        <text
                                            x={polarToCartesian(cx, cy, innerVoidRadius - 12, angle).x}
                                            y={polarToCartesian(cx, cy, innerVoidRadius - 12, angle).y}
                                            textAnchor="middle"
                                            dominantBaseline="central"
                                            style={{
                                                fontFamily: "'DM Sans', system-ui, sans-serif",
                                    ...
                                    {/* Número da casa natal — pequeno, dourado, dentro do anel */}
                                    <text
                                        x={labelPos.x}
                                        y={labelPos.y}
                                        textAnchor="middle"
                        ...
                        {/* Círculo interno tracejado delimitando o anel de casas natais */}
                        <circle
                            cx={cx} cy={cy}
                            r={innerVoidRadius}
                            fill="none"
                ...
                <g>
                    {houseCusps.map((cusp, i) => {
                        ...
                        else if (houseNumber === 4) label = 'IC';
                        else if (houseNumber === 7) label = 'DSC';
                            ...
                            <g key={`house-group-${i}`}>
                                ...
                                {showHouseMarkers && (
                                    <text 
                                        ...
                                        className={`font-bold select-none cursor-pointer ${(['ASC', 'MC', 'IC', 'DSC'].includes(label) || houseFormat === 'roman') ? 'font-display' : 'font-ui'}`}
                                        style={{ 
                                            fill: houseColor,
                                            fontSize: '16px', 
                                            pointerEvents: 'auto', 
                                            textShadow: 'none',
                                            fontFamily: ['ASC', 'MC', 'IC', 'DSC'].includes(label) 
                                                ? "'Cormorant Garamond', serif" 
                                                : (houseFormat === 'roman' ? "'Cormorant Infant', serif" : "'Manrope', sans-serif")
                                        }}
                                        onClick={() => onHouseClick(houseNumber)}
                                        onMouseEnter={(e) => handleMouseEnter(e, 'house', houseNumber)}
                                        onMouseMove={handleMouseMove}
                ...
                >
                    <text x={cx} y={cy - 42} textAnchor="middle"
                        style={{
                            fill: theme === 'light' ? '#355170' : '#9CA3AF',
                    ...
                    {/* Hora — PT Serif italic 400 */}
                    <text x={cx} y={cy -12} textAnchor="middle" dominantBaseline="middle"
                        style={{
                            fill: theme === 'light' ? '#1a2e44' : '#F5F2EB',
                            fontFamily: "'PT Serif', Georgia, serif",
                            fontStyle: 'italic',
                            fontWeight: 400,
                            fontSize: '32px',
                    ...
                    {/* Data — DM Sans uppercase */}
                    <text x={cx} y={cy + 28} textAnchor="middle"
                        style={{
                            fill: theme === 'light' ? '#2d4a6a' : '#D1D5DB',
                            fontFamily: "'DM Sans', system-ui, sans-serif",
                            fontWeight: 500,
                            fontSize: '18px',
                            letterSpacing: '0.11em',
                        }}>
                        {String(dayVal).padStart(2,'0')} · {monthVal} · {yearVal}
                    </text>

                    {/* Local — DM Sans, abaixo da data */}
               ...
               {filteredAspects.length > 0 && (showAspectLines || (isNatalMode && showNatalLines)) && (
                    <g className="opacity-90" mask="url(#center-mask)">
                        {filteredAspects.map((aspect) => {
                            ...
                            if (isAnyAspectHovered) {
                                ...
                                if (isSelected) {
                                    isVisible = true;
                                    strokeOpacity = Math.max(0.9, strokeOpacity);
                    ...
                    {sortedPlanetIds.map(planetId => {
                        ...
                        return (
                            <g 
                                ...
                                className="cursor-pointer group !transition-none" style={{ opacity, pointerEvents: pointerEvents as any, transform: `translate(${x}px, ${y}px)`, willChange: 'transform', transition: smoothTransitions ? 'transform 1s linear, opacity 0.3s ease' : 'opacity 0.3s ease' }}
                            >
                                <g>
                                    ...
                                    {showDegreeLabels && (
                                        <text
                                            ...
                                            textAnchor="start"
                                            style={{
                                                fill: theme === 'light' ? '#355170' : '#94a3b8',
                ...
                {natalData && (
                    <g className="pointer-events-none">
                        {sortedPlanetIds.map(planetId => {
                            ...
                            return (
<g key={`natal-${planetId}`}
    style={{
        transform: `translate(${x}px, ${y}px)`,
        transition: 'none',
    }}
    opacity={0.95}>                                    <text x="0" y="0" dy={fontSize * 0.35} textAnchor="middle" fill={theme === 'light' ? '#92400E' : '#FFE566'} stroke={theme === 'light' ? '#78350F' : '#C8960B'} strokeWidth="0.3" fontSize={fontSize * 1.2} style={{ fontFamily: '"Noto Sans Symbols", "Segoe UI Symbol", sans-serif' }}>
                                        {CELESTIAL_GLYPHS[planetId]}
                                    </text>
                                </g>
                            );
